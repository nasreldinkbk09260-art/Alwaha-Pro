...
declare
    ...
    users_is_view boolean := false;
begin
    -- إذا كان public.users عبارة عن View حالياً، نقوم بحذفه أولاً لمنع تعارض ALTER TABLE
    select exists (
        select 1 from information_schema.views
        where table_schema = 'public' and table_name = 'users'
    ) into users_is_view;

    if users_is_view then
...
-- -----------------------------------------------------------------------------
-- 13) Storage bucket and policies.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 16) Backfill/repair Auth profiles.
-- -----------------------------------------------------------------------------
insert into public.users (
