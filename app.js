...
// إعدادات Supabase الخاصة بالمشروع
...
// دالة برمجية مخصصة لإجبار المتصفح على فتح معرض الصور (الاستوديو)
...
// معالجة اختيار صورة الصورة الشخصية للبروفايل عند التسجيل
function handleAvatarImageSelection(input) {
...
function syncUiWithLoadedProfileData() {
    const name = localStorage.getItem('alwaha_profile_name') || "زائر واحة المحترفين";
    const email = localStorage.getItem('alwaha_profile_email') || "guest@alwaha.pro";
...
async function handleNewUserRegistrationSubmit() {
    ...
    const email = document.getElementById('reg-identifier')?.value.trim() || "بدون_بريد";
        ...
        container.innerHTML += `
            <div class="news-box text-right" style="border-right: 3px solid ${post.post_type === 'تقني' ? '#00ffff' : '#ff00ff'}; background:rgba(0,0,0,0.3); padding:15px; border-radius:8px; margin-bottom:15px;">
                ...
                <div class="post-actions-row" style="display:flex; gap:15px; margin-top:10px; border-top:1px solid rgba(255,255,255,0.1); padding-top:10px;">
                    ...
                    <button class="btn-action" style="background:transparent; color:#fff; padding:4px 10px; font-size:12px; flex:1;" onclick="triggerToastNotification('سيتم فتح نافذة التعليقات', 'info')">
...
// ==========================================
// 10. الأقسام الجديد: الريلز والألعاب
// ==========================================
        ...
        container.innerHTML += `
            <div class="reel-card" style="position:relative; height:400px; background:#222; border-radius:15px; overflow:hidden; margin-bottom:20px; display:flex; align-items:center; justify-content:center;">
                ...
                <div style="position:absolute; right:15px; bottom:30px; display:flex; flex-direction:column; gap:15px; align-items:center;">
                    ...
                    <button class="reel-circle-btn" style="background:rgba(255,255,255,0.2); border:none; width:40px; height:40px; border-radius:50%; color:#fff; font-size:18px;" onclick="triggerToastNotification('قسم التعليقات', 'info')"><i class="fa-solid fa-comment-dots"></i></button>
...
// تهيئة النظام عند اكتمال تحميل عناصر الصفحة
