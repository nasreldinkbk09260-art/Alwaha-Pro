...
// ==========================================
// 13. الملف الشخصي وتخصيص الثيمات
// ==========================================
...
function saveUserSettingsData() {
    ...
    if(name) localStorage.setItem('alwaha_profile_name', name);
    if(email) localStorage.setItem('alwaha_profile_email', email);
    if(phone) localStorage.setItem('alwaha_profile_phone', phone);

    alert("✨ تم حفظ التغيرات الجديدة في الحساب بنجاح.");
...
function syncUiWithLoadedProfileData() {
    ...
    if(avatarPlaceholder) {
        if(savedAvatar) {
            avatarPlaceholder.innerHTML = `<img src="${savedAvatar}" alt="Avatar" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`;
        } else {
...
// تشغيل المحرك والتحقق الأولي فور التحميل
